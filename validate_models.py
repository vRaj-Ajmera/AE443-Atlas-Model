...
def validate_model(validation_file, prediction_function, input_columns, output_column):
    ...
    # Add errors to DataFrame and save results
    df['Predicted'] = [prediction_function(*[row[col] for col in input_columns]) for _, row in df.iterrows()]
    df['Error (%)'] = errors

    # Save validation results to a new CSV
    output_file = validation_file.replace(".csv", "_validation_results.csv")
...
def main():
    ...
    validation_tasks = [
        ("val/payload_maneuverability.csv", predict_payload_maneuverability, ["payload_mass", "robot_power", "terrain_complexity"], "travel_speed"),
        ("val/crew_time.csv", predict_crew_time, ["task_complexity", "automation_level"], "crew_time_minutes"),
        ("val/position_accuracy.csv", predict_position_accuracy, ["power_usage", "environmental_noise", "path_variability"], "positional_error")
    ]
